...
//    hg_bool_t cmp_result = HG_Addr_cmp(hvac_comm_get_class(), hgi->addr, server_address);
    ...
    */
    ...
    if (hvac_rpc_state_p->in.offset == -1){
        ...
        string filename = "/proc/self/fd/" + to_string(hvac_rpc_state_p->in.accessfd); 
        ...
        L4C_INFO("Server Rank %d : Read %ld bytes from file %s, fd: %d", server_rank,readbytes, filename.c_str(), hvac_rpc_state_p->in.accessfd);
