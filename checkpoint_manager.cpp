...
{
  allocate_new_chunk();
  worker_thread = std::thread(&CheckpointManager::background_worker, this);
}
...
void CheckpointManager::write_checkpoint(const std::string &filename, const void *buf, size_t count, int local_fd)
{
  ...
  {
    std::lock_guard<std::mutex> lock(queue_mutex);
    data_queue.emplace(filename, std::move(data));
  }
  cv.notify_one();
}

void CheckpointManager::background_worker()
{
  while (true)
  {
    std::pair<std::string, std::vector<char>> task;
    {
      std::unique_lock<std::mutex> lock(queue_mutex);
      cv.wait(lock, [this]()
              { return !data_queue.empty() || stop_worker; });

      if (stop_worker && data_queue.empty())
        break;

      task = std::move(data_queue.front());
      data_queue.pop();
    }

    const auto &filename = task.first;
    const char *data = task.second.data();
    size_t remaining = task.second.size();
